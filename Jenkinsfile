...
pipeline {
    ...
    environment {
        ...
        CONTAINER_NAME = "jenkinsdemo-app-container"
        DOCKERHUB_USER = "deborahdel"
        SONARQUBE_HOST = "http://localhost:9000"         // adjust to your SonarQube server
    ...
    stages {
        ...
        stage('Delete Previous SonarQube Container') {
            steps {
                echo "🗑️ Deleting previous SonarQube container if it exists..."
                sh '''
                    if [ $(docker ps -aq -f name=sonarqube) ]; then
                        ...
                        echo "No SonarQube container found. Skipping cleanup."
                    fi
                '''
            }
        }
